...
namespace Assets.script
{
    ...
    public class Graph
    {
        ...
        public bool Astar(int ini, int fin, ref List<int> res, ref List<int> reco, ref Text E1, ref Text E2)
        {
            ...
            Node n_fin = findNode(fin);
            close.Add(n_ini);
            List<Node> chl = new List<Node>();
            res.Add(n_ini.val);
            reco.Add(n_ini.val);
            for (int i = 0; i < n_ini.edges.Count(); i++)
            {
                if (!n_ini.edges[i].ignorado)
                {
                    n_ini.edges[i].father = n_ini;
                    chl.Add(n_ini.edges[i]);
                    reco.Add(n_ini.edges[i].val);
                    n_ini.edges[i].color = 1;
                }
            }
            double dm = 10000;
            int aux = 0;
            ...
            {
                if (helper.distanceNodes(chl[i], n_fin) < dm)
                {
                    dm = helper.distanceNodes(chl[i], n_fin);
                    aux = i;
                }
            }
            open.Add(chl[aux]);
            while (open.Count() != 0)
            {
                ...
                dm = 10000;
                chl.Clear();
                for(int i = 0; i < open[0].edges.Count(); i++)
                {
                    if (!open[0].edges[i].ignorado && open[0].edges[i].color != 1)
                    {
                        open[0].edges[i].father = open[aux];
                        reco.Add(open[0].edges[i].val);
                        chl.Add(open[0].edges[i]);
                        open[0].edges[i].color = 1;
